...
func (k KothStart) Run(s cmd.Source, o *cmd.Output) {
	...
	internal.Broadcastf("koth.start", name, ko.Name(), coords.X(), coords.Y())
	var st string
	if ko == koth.Citadel {
		st = fmt.Sprintf(`
 §e█████████§r
...
func (KothStop) Run(s cmd.Source, o *cmd.Output) {
	...
	if k, ok := koth.Running(); !ok {
		...
		k.Stop()
		internal.Broadcastf("koth.stop", name, k.Name())
	}
}
...
func (kothList) Options(src cmd.Source) []string {
	...
	for _, k := range koth.All() {
		...
		opts = append(opts, colour.Strip(strings.ToLower(k.Name())))
	}
	return opts
}
