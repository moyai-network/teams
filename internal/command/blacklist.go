...
// Run ...
func (b BlacklistLiftOffline) Run(src cmd.Source, o *cmd.Output) {
	l := locale(src)
...
// Run ...
func (b Blacklist) Run(src cmd.Source, o *cmd.Output) {
	l := locale(src)
...
// Run ...
func (b BlacklistOffline) Run(src cmd.Source, o *cmd.Output) {
	l := locale(src)
	s := src.(cmd.NamedTarget)
	if s.Name() == b.Target {
		o.Error(lang.Translate(l, "command.blacklist.self"))
