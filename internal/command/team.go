...
func (t TeamCreate) Run(src cmd.Source, _ *cmd.Output) {
	...
	internal.Broadcastf("team.create.success.broadcast", p.Name(), tm.DisplayName)
}
...
func (t TeamInformation) Run(src cmd.Source, out *cmd.Output) {
	...
	if name == "" {
		...
		out.Print(formatTeamInformation(tm))
		return
	}
...
func (t TeamHome) Run(src cmd.Source, _ *cmd.Output) {
	...
	if area.Spawn(p.World()).Vec3WithinOrEqualXZ(p.Position()) {
		if p.World() != internal.Overworld() {
			internal.Overworld().AddEntity(p)
		}
		p.Teleport(tm.Home)
		return
	}
...
func formatTeamInformation(t data.Team) string {
