...
func (menes) Rewards() []Reward {
	return []Reward{
		...
		NewReward(item.NewStack(item.Sword{Tier: item.ToolTierDiamond}, 1).WithEnchantments(
			item.NewEnchantment(ench.Sharpness{}, 2), item.NewEnchantment(enchantment.Unbreaking{}, 2), item.NewEnchantment(enchantment.FireAspect{}, 1)).WithCustomName(text.Colourf("<emerald>Menes Sword</emerald>")), 10),
		...
		18: NewReward(it.NewSpecialItem(it.PartnerPackageType{}, 1), 5),
		19: NewReward(it.NewSpecialItem(it.PartnerPackageType{}, 3), 14),
		20: NewReward(it.NewSpecialItem(it.PartnerPackageType{}, 5), 3),
		21: NewReward(it.NewSpecialItem(it.PartnerPackageType{}, 7), 2),
		22: NewReward(it.NewSpecialItem(it.PartnerPackageType{}, 9), 1),
