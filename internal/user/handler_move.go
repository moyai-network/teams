...
import (
	...
	b "github.com/moyai-network/teams/internal/block"
...
func (h *Handler) HandleMove(ctx *event.Context, newPos mgl64.Vec3, newYaw, newPitch float64) {
	...
	if ar := area.Spawn(w); ar.Area != (area.Area{}) && ar.Vec3WithinOrEqualFloorXZ(newPos) && h.p.World() != internal.Deathban() && h.tagCombat.Active() || err != nil || u.Frozen {
		ctx.Cancel()
		return
	}
...
func (h *Handler) updateWalls(ctx *event.Context, newPos mgl64.Vec3, u data.User) {
	...
	if h.tagCombat.Active() {
		...
		if mul.Vec3WithinOrEqualFloorXZ(p.Position()) {
			h.sendWall(cubePos, a.Area, item.ColourRed())
		}
	}
