...
func (h *Handler) HandleHurt(ctx *event.Context, dmg *float64, im *time.Duration, src world.DamageSource) {
	...
	*dmg = *dmg / 1.25
	...
	if area.Spawn(h.p.World()).Vec3WithinOrEqualFloorXZ(h.p.Position()) && h.p.World() != internal.Deathban() {
		ctx.Cancel()
		return
	}

	if u.Teams.DeathBan.Active() && area.Deathban.Spawn().Vec3WithinOrEqualFloorXZ(h.p.Position()) {
		ctx.Cancel()
		return
	}
	...
	if (p.Health()-p.FinalDamageFrom(*dmg, src) <= 0 || (src == entity.VoidDamageSource{})) && !ctx.Cancelled() {
		...
		if ok {
			...
			if k.Teams.Stats.KillStreak%5 == 0 {
				internal.Broadcastf("user.killstreak", killer.Name(), k.Teams.Stats.KillStreak)
