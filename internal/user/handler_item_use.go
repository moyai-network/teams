...
import (
	class2 "github.com/moyai-network/teams/internal/user/class"
	"math/rand"
	"time"
...
func (h *Handler) HandleItemUse(ctx *event.Context) {
	...
	if v, ok := it.SpecialItem(held); ok {
		...
		if h.lastArea.Load().Name() == koth.Citadel.Name() {
			internal.Messagef(h.p, "item.use.citadel.disabled")
			ctx.Cancel()
			return
		}
		if h.coolDownGlobalAbilities.Active() {
			internal.Messagef(h.p, "partner_item.cooldown", h.coolDownGlobalAbilities.Remaining().Seconds())
			ctx.Cancel()
		} else {
			h.handleSpecialItemUse(v, held, left, ctx)
		}
...
func (h *Handler) handleArcherRogueItemUse(held item.Stack, left item.Stack) {
	...
	if e, ok := ArcherRogueEffectFromItem(held.Item()); ok {
		...
		if cd := h.coolDownArcherRogueItem.Key(held.Item()); cd.Active() {
			internal.Messagef(h.p, "class.ability.cooldown", cd.Remaining().Seconds())
			return
		}
...
func (h *Handler) handleBardItemUse(held item.Stack, sotwRunning bool, u data.User) {
	...
	if e, ok := BardEffectFromItem(held.Item()); ok {
		...
		if en := h.energy.Load(); en < 30 {
			internal.Messagef(h.p, "class.energy.insufficient")
			return
		} else {
		...
		internal.Messagef(h.p, "bard.ability.use", effectutil.EffectName(e), lvl, len(teammates))
		h.p.SetHeldItems(held.Grow(-1), item.Stack{})
...
func (h *Handler) handleMageItemUse(held item.Stack, sotwRunning bool, u data.User) {
	...
	if e, ok := MageEffectFromItem(held.Item()); ok {
		...
		// Notify player about ability usage.
		lvl, _ := roman.Itor(e.Level())
		internal.Messagef(h.p, "mage.ability.use", effectutil.EffectName(e), lvl, len(enemies))
		h.p.SetHeldItems(held.Grow(-1), item.Stack{})
...
func (h *Handler) handleRandomTeleport() {
	...
	internal.Messagef(h.p, "command.teleport.self", target.Name())
}
...
func (h *Handler) handlePearlUse(ctx *event.Context) {
	if cd := h.coolDownPearl; cd.Active() {
		internal.Messagef(h.p, "pearl.cooldown", cd.Remaining().Seconds())
		ctx.Cancel()
		return
	}
...
func (h *Handler) handleTimeWarpAbility(kind it.TimeWarpType) {
	...
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Time Warp", cd.Remaining().Seconds())
		return
	}
	internal.Messagef(h.p, "partner_item.used", "Time Warp")
...
func (h *Handler) handleSwitcherBallAbility(kind it.SwitcherBallType, ctx *event.Context) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Switcher Ball", cd.Remaining().Seconds())
		ctx.Cancel()
		return
	}
	h.coolDownGlobalAbilities.Set(time.Second * 10)
...
func (h *Handler) handleFullInvisibilityAbility(kind it.FullInvisibilityType, held item.Stack, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Full Invisibility", cd.Remaining().Seconds())
		return
	}
	h.ShowArmor(false)
	...
	internal.Messagef(h.p, "partner_item.used", "Full Invisibility")
}
...
func (h *Handler) handleCloseCallAbility(kind it.CloseCallType, held item.Stack, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Close Call", cd.Remaining().Seconds())
		return
	}
	...
	if h.p.Health() <= 8 {
		...
		h.p.Message()
		internal.Messagef(h.p, "partner_item.used", "Close Call")
	}
}
...
func (h *Handler) handleBeserkAbility(kind it.BeserkAbilityType, held item.Stack, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Beserk Ability", cd.Remaining().Seconds())
		return
	}
	...
	internal.Messagef(h.p, "partner_item.used", "Beserk Ability")
}
...
func (h *Handler) handleNinjaStarAbility(kind it.NinjaStarType) {
	...
	if !ok {
		internal.Messagef(h.p, "partner_item.last_hit")
		return
	}
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Ninja Star", cd.Remaining().Seconds())
		return
	}

	internal.Messagef(h.p, "partner_item.used.teleporting", "Ninja Star", lastAttacker.Name())
	internal.Messagef(lastAttacker, "ninja_star.teleporting", h.p.Name())
	h.coolDownGlobalAbilities.Set(time.Second * 10)
	h.coolDownSpecificAbilities.Set(kind, time.Minute*2)
...
func (h *Handler) handleFocusModeAbility(kind it.FocusModeType, held item.Stack, left item.Stack) {
	...
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Focus Mode", cd.Remaining().Seconds())
		return
	}
	...
	if t, ok := lastAttacker.Handler().(*Handler); ok {
		...
		h.p.SetHeldItems(subtractItem(h.p, held, 1), left)
		internal.Messagef(h.p, "partner_item.used.on", "Focus Mode", t.p.Name())
		internal.Messagef(t.p, "focus_mode.used", h.p)
	}
}
...
func (h *Handler) handleRocketAbility(kind it.RocketType) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Rocket", cd.Remaining().Seconds())
		return
	}
...
func (h *Handler) handleAbilityDisablerAbility(kind it.AbilityDisablerType, held item.Stack, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Ability Disabler", cd.Remaining().Seconds())
		return
	}
	...
	for _, e := range enemies {
		...
		internal.Messagef(e.p, "ability_disabler.used", h.p.Name())
	}

	internal.Messagef(h.p, "partner_item.used", "Ability Disabler")
}

// handleAbilityDisablerAbility handles the ability disabler ability
func (h *Handler) handleStrengthPowderAbility(kind it.StrengthPowderType, held item.Stack, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Strength Powder", cd.Remaining().Seconds())
		return
	}
	...
	internal.Messagef(h.p, "partner_item.used", "Strength Powder")
}

func (h *Handler) handleTankIngotAbility(kind it.TankIngotType, held item.Stack, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Tank Ingot", cd.Remaining().Seconds())
		return
	}
	h.coolDownGlobalAbilities.Set(time.Second * 10)
	h.coolDownSpecificAbilities.Set(kind, time.Minute*2)
	...
	internal.Messagef(h.p, "partner_item.used", "Tank Ingot")
}

func (h *Handler) handleRageBrickAbility(kind it.RageBrickType, held item.Stack, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Rage Brick", cd.Remaining().Seconds())
		return
	}
	h.coolDownGlobalAbilities.Set(time.Second * 10)
	h.coolDownSpecificAbilities.Set(kind, time.Minute*2)
	...
	internal.Messagef(h.p, "partner_item.used", "Rage Brick")
}

func (h *Handler) handleComboAbility(kind it.ComboAbilityType, held item.Stack, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Combo Ability", cd.Remaining().Seconds())
		return
	}
	h.coolDownGlobalAbilities.Set(time.Second * 10)
	h.coolDownSpecificAbilities.Set(kind, time.Minute*2)
	...
	internal.Messagef(h.p, "partner_item.used", "Combo Ability")
}

func (h *Handler) handleVampireAbility(kind it.VampireAbilityType, held item.Stack, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(kind); cd.Active() {
		internal.Messagef(h.p, "partner_item.item.cooldown", "Vampire Ability", cd.Remaining().Seconds())
		return
	}
	h.coolDownGlobalAbilities.Set(time.Second * 10)
	h.coolDownSpecificAbilities.Set(kind, time.Minute*2)
	...
	internal.Messagef(h.p, "partner_item.used", "Vampire Ability")
}
