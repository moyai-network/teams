...
import (
	...
	blck "github.com/moyai-network/teams/internal/block"
...
func (h *Handler) HandleItemUseOnBlock(ctx *event.Context, pos cube.Pos, face cube.Face, clickPos mgl64.Vec3) {
	...
	if crateFound {
		...
		if !canOpenCrate(held, c) {
			internal.Messagef(h.p, "crate.key.require", colour.Strip(c.Name()))
			return
		}
	...
	case item.Hoe:
		...
		if crowbar {
			...
			if posWithinProtected {
				internal.Messagef(h.p, "team.claim.not-within")
				break
			}
		...
		if h.p.World() != internal.Overworld() {
			break
		}

		if !tm.Leader(h.p.Name()) {
			internal.Messagef(h.p, "team.not-leader")
			break
		}

		if tm.Claim != (area.Area{}) {
			internal.Messagef(h.p, "team.has-claim")
			break
		}
		...
		for _, t := range teams {
			...
			if areaTooClose(c, vec3ToVec2(pos.Vec3()), 1) {
				internal.Messagef(h.p, "team.area.too-close")
				return
			}
		}
		...
		if h.p.Sneaking() {
			...
			cost := int(a * 2)
			internal.Messagef(h.p, "team.claim.cost", cost)
		}
		...
		internal.Messagef(h.p, "team.claim.set-position", pn, mgl64.Vec2{float64(pos.X()), float64(pos.Z())})
	default:
		if !h.p.Sneaking() {
			break
		}
	...
	case block.WoodFenceGate, block.Chest, block.WoodTrapdoor, block.WoodDoor, block.ItemFrame, block.Hopper, block.Beacon:
		...
		if h.coolDownBonedEffect.Active() {
			internal.Messagef(h.p, "user.interaction.boned")
			ctx.Cancel()
			return
		}
	case block.Sign:
		...
		title := strings.ToLower(colour.Strip(lines[0]))
		body := strings.ToLower(colour.Strip(lines[1]))
		...
		if body == "have lives?" {
			...
			if u.Teams.Lives <= 0 {
				internal.Messagef(h.p, "lives.none")
				return
			}
			if u.Teams.DeathBan.Active() {
				...
				data.SaveUser(u)
				internal.Overworld().AddEntity(h.p)
				h.p.Armour().Clear()
				h.p.Inventory().Clear()
...
func canOpenCrate(held item.Stack, c crate.Crate) bool {
	_, ok := held.Value("crate-key_" + colour.Strip(c.Name()))
	return ok
}
