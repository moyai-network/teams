...
func (h *Handler) handleRogueBackstab(ctx *event.Context, t *player.Player, force, height float64) {
	...
	if s, ok := held.Item().(item.Sword); ok && s.Tier == item.ToolTierGold && class2.Compare(h.lastClass.Load(), class2.Rogue{}) && t.Rotation().Direction() == h.p.Rotation().Direction() {
...
func (h *Handler) handlePearlDisablerAbility(targetPlayer *player.Player, held, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(it.PearlDisablerType{}); cd.Active() {
		internal.Messagef(h.p, "pearl_disabler.cooldown", cd.Remaining().Seconds())
		return
	}

	internal.Messagef(targetPlayer, "pearl_disabler.target", h.p.Name())
	targetHandler, ok := targetPlayer.Handler().(*Handler)
	if !ok {
		return
	}
	...
	internal.Messagef(h.p, "pearl_disabler.user", targetPlayer.Name())
...
func (h *Handler) handleExoticBoneAbility(targetPlayer *player.Player, held, left item.Stack) {
	if cd := h.coolDownSpecificAbilities.Key(it.ExoticBoneType{}); cd.Active() {
		internal.Messagef(h.p, "bone.cooldown", cd.Remaining().Seconds())
		return
	}

	internal.Messagef(targetPlayer, "bone.target", h.p.Name())
	internal.Messagef(h.p, "bone.user", targetPlayer.Name())
...
func (h *Handler) handleSpecialCases(targetPlayer *player.Player, ctx *event.Context) bool {
	if colour.Strip(targetPlayer.Name()) == "Click to use kits" {
		if mn, ok := menu.NewKitsMenu(h.p); ok {
			inv.SendMenu(h.p, mn)
		}
		ctx.Cancel()
		return true
	} else if colour.Strip(targetPlayer.Name()) == "Block Shop" {
