...
func (d Diamond) Items(*player.Player) [36]item.Stack {
	...
	items := [36]item.Stack{
		item.NewStack(item.Sword{Tier: item.ToolTierDiamond}, 1).WithEnchantments(item.NewEnchantment(enchantment2.Sharpness{}, lvl)),
...
func (d Diamond) Armour(*player.Player) [4]item.Stack {
	...
	protection := item.NewEnchantment(enchantment2.Protection{}, lvl)
	unbreaking := item.NewEnchantment(enchantment.Unbreaking, 3)

	nightVision := item.NewEnchantment(enchantment2.NightVision{}, 1)
	fireRes := item.NewEnchantment(enchantment2.FireResistance{}, 1)
	recovery := item.NewEnchantment(enchantment2.Recovery{}, 1)
	...
	if !d.Free {
		...
		bootsEnchants = append(bootsEnchants, item.NewEnchantment(enchantment2.Speed{}, 2))
	}
