...
func (h *Handler) SetWayPoint(p *player.Player, w *WayPoint) {
	waypointMu.Lock()
	defer waypointMu.Unlock()
	id, _ := uuid.NewRandom()
	name := text.Colourf("<purple>%s</purple> [%.1fm]", w.name, h.distanceToWaypoint(p))
	entityCount += 1
...
func (h *Handler) RemoveWaypoint(p *player.Player) {
	...
	unsafe.WritePacket(p, &packet.MovePlayer{
...
func (h *Handler) updateWaypointPosition(p *player.Player) {
	if h.waypoint == nil {
		return
	}
	...
	move := &packet.MovePlayer{
		EntityRuntimeID: h.waypoint.entityRuntimeID,
		Position:        h.clientDisplayPosition(p),
		Mode:            packet.MoveModeNormal,
	}

	meta := protocol.NewEntityMetadata()
	meta[protocol.EntityDataKeyName] = text.Colourf("<purple>%s</purple> [%.1fm]", h.waypoint.name, h.distanceToWaypoint(p))
	...
	unsafe.WritePacket(p, move)
	unsafe.WritePacket(p, set)
}

func (h *Handler) clientDisplayPosition(p *player.Player) mgl32.Vec3 {
	var clientPos mgl64.Vec3
	if h.distanceToWaypoint(p) > 10 {
		clientPos = p.Position().Add(h.waypoint.position.Sub(p.Position()).Normalize().Mul(10))
	} else {
		clientPos = h.waypoint.position
	}
	height := p.EyeHeight() * 2
	clientPos = clientPos.Add(mgl64.Vec3{0, height})
	return vec64To32(clientPos)
}

func (h *Handler) distanceToWaypoint(p *player.Player) float64 {
	if h.waypoint == nil {
		return 0
	}
