...
func (c Colour) Run(source cmd.Source, output *cmd.Output, tx *world.Tx) {
	...
	if held.Empty() {
		internal.Messagef(p, "command.colour.hold")
		return
	}
	var clr item.Colour
	...
	default:
		internal.Messagef(p, "command.colour.invalid")
	}
	...
	case item.Helmet:
		if _, ok := it.Tier.(item.ArmourTierLeather); !ok {
			internal.Messagef(p, "command.colour.leather")
			return
		}
		it.Tier = newTier
		newItem = it
