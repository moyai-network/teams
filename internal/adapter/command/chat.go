...
// Run ...
func (c ChatMute) Run(s cmd.Source, o *cmd.Output, tx *world.Tx) {
...
// Run ...
func (c ChatUnMute) Run(s cmd.Source, o *cmd.Output, tx *world.Tx) {
	p, ok := s.(*player.Player)
	if !ok {
		return
	}
...
// Run ...
func (c ChatCoolDown) Run(s cmd.Source, o *cmd.Output, tx *world.Tx) {
	p, ok := s.(*player.Player)
	if !ok {
		return
	}
