...
// Run ...
func (MuteList) Run(s cmd.Source, o *cmd.Output, tx *world.Tx) {
	// l := locale(s)
...
// Run ...
func (m MuteInfo) Run(s cmd.Source, o *cmd.Output, tx *world.Tx) {
	l := locale(s)
...
// Run ...
func (m MuteLift) Run(src cmd.Source, out *cmd.Output, tx *world.Tx) {
	l := locale(src)
	p, ok := m.Targets[0].(*player.Player)
	if !ok {
...
// Run ...
func (m MuteForm) Run(s cmd.Source, _ *cmd.Output, tx *world.Tx) {
	// p := s.(*player.Player)
	// p.SendForm(form.NewMute(p))
}

// Run ...
func (m Mute) Run(src cmd.Source, out *cmd.Output, tx *world.Tx) {
	l := locale(src)
	if len(m.Targets) > 1 {
...
// Run ...
func (m MuteOffline) Run(src cmd.Source, out *cmd.Output, tx *world.Tx) {
	l := locale(src)
	sn := src.(cmd.NamedTarget)
