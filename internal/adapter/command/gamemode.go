...
// Run ...
func (g GameMode) Run(src cmd.Source, _ *cmd.Output, tx *world.Tx) {
	var name string
	...
	if len(targets) == 1 {
		...
		internal.Alertf(tx, src, "staff.alert.gamemode.change.other", target.Name(), name)
		internal.Messagef(src, "command.gamemode.update.other", target.Name(), name)
		return
	}
	if p, ok := src.(*player.Player); ok {
		p.SetGameMode(mode)
		internal.Alertf(tx, src, "staff.alert.gamemode.change", name)
		internal.Messagef(src, "command.gamemode.update.self", name)
		return
	}
	internal.Messagef(src, "command.gamemode.console")
}
