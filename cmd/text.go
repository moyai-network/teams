...
func placeText(c internal.Config) {
	...
	for _, e := range internal.Overworld().Entities() {
	...
	for i := 0; i < count; i++ {
		for _, e := range []*entity.Ent{
			...
			entity.NewText(text.Colourf("<red>Discord</red>: discord.internal.club"), mgl64.Vec3{spawn.X(), spawn.Y() + 0.5, spawn.Z()}),
			entity.NewText(text.Colourf("<grey>internal.club</grey>"), spawn),
		} {
			w.AddEntity(e)
		}
		...
		for _, e := range []*entity.Ent{
			...
			entity.NewText(text.Colourf("<red>Buy Keys</red>: store.internal.club"), mgl64.Vec3{crate.X(), crate.Y() + 0.5, crate.Z()}),
		} {
			w.AddEntity(e)
		}
		...
		for _, e := range []*entity.Ent{
			...
			entity.NewText(text.Colourf("<red>Buy Kits</red>: store.internal.club"), mgl64.Vec3{kit.X(), kit.Y() + 0.5, kit.Z()}),
		} {
			w.AddEntity(e)
		}
		...
		for _, e := range []*entity.Ent{
			...
			entity.NewText(text.Colourf("<red>Buy Credits</red>: store.internal.club"), mgl64.Vec3{shop.X(), shop.Y() + 0.5, shop.Z()}),
		} {
			w.AddEntity(e)
		}
		...
		for _, e := range []*entity.Ent{
			...
			entity.NewText(text.Colourf("<red>Buy Ranks</red>: store.internal.club"), mgl64.Vec3{adv.X(), adv.Y() + 0.5, adv.Z()}),
		} {
			w.AddEntity(e)
		}
	}
