...
func (t StatsOnlineCommand) Run(s cmd.Source, o *cmd.Output) {
	targets := t.Targets.LoadOr(nil)

	if len(targets) > 1 {
		moyai.Messagef(s, "command.targets.exceed")
		return
	}
	...
	if len(targets) == 1 {
		...
		if !ok {
			moyai.Messagef(s, "command.target.unknown", "")
			return
		}
		name = target.Name()
	} else {
...
func (s StatsOfflineCommand) Run(src cmd.Source, o *cmd.Output) {
	u, err := data.LoadUserFromName(s.Target)
	if err != nil {
		moyai.Messagef(src, "command.target.unknown", s.Target)
		return
	}
