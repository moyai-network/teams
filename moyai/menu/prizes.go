...
var (
	advancedRewards = []item.Stack{
		it.NewSpecialItem(it.PartnerPackageType{}, 25),
		it.NewKey(it.KeyTypePharaoh, 25),
	}

	rewards = []reward{
		// 1 hour
		{slot: 4, requiredPlayTime: time.Hour, rewards: []item.Stack{
			it.NewSpecialItem(it.PartnerPackageType{}, 5),
			it.NewKey(it.KeyTypePharaoh, 5),
		}},
		// 3 hours
		{slot: 15, requiredPlayTime: time.Hour * 3, rewards: []item.Stack{
			it.NewSpecialItem(it.PartnerPackageType{}, 10),
			it.NewKey(it.KeyTypePharaoh, 10),
		}},
		// 5 hours
		{slot: 34, requiredPlayTime: time.Hour * 5, rewards: []item.Stack{
			it.NewSpecialItem(it.PartnerPackageType{}, 15),
			it.NewKey(it.KeyTypePharaoh, 15),
		}},
		// 7 hours
		{slot: 41, requiredPlayTime: time.Hour * 7, rewards: []item.Stack{
			it.NewSpecialItem(it.PartnerPackageType{}, 20),
			it.NewKey(it.KeyTypePharaoh, 20),
		}},
		// 10 hours
